...
public class UnCompletedChallengesFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v =  inflater.inflate(R.layout.fragment_un_completed_challenges, container, false);
        session = new Session(getContext());
        user_id = session.getUserID();

        rvItems = v.findViewById(R.id.unCompletedChallengesRecyclerView);
        linearLayoutManager = new LinearLayoutManager(getContext());
        rvItems.setLayoutManager(linearLayoutManager);
        RetrofitClient.apiServices().getUncompletedChallengesList(user_id, ChallengeConstants.UNCOMPLETED,0).enqueue(new Callback<List<ChallengeSummary>>() {
            @Override
    ...
    public void loadNextDataFromApi(int offset) {
        ...
        RetrofitClient.apiServices().getUncompletedChallengesList(user_id, ChallengeConstants.UNCOMPLETED,offset).enqueue(new Callback<List<ChallengeSummary>>() {
            @Override
            public void onResponse(Call<List<ChallengeSummary>> call, Response<List<ChallengeSummary>> response) {
                if(response.isSuccessful()){
