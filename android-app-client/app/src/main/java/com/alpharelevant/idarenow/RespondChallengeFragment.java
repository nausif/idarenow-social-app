...
public class RespondChallengeFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v =  inflater.inflate(R.layout.fragment_respond_challenge, container, false);
        session = new Session(getContext());
        user_id = session.getUserID();

        rvItems = v.findViewById(R.id.RespondChallengesRecyclerView);
        linearLayoutManager = new LinearLayoutManager(getContext());
        rvItems.setLayoutManager(linearLayoutManager);
        RetrofitClient.apiServices().getRespondChallengesList(user_id, ChallengeConstants.INITIAL,0).enqueue(new Callback<List<ChallengeSummary>>() {
            @Override
    ...
    public void loadNextDataFromApi(int offset) {
        ...
        RetrofitClient.apiServices().getRespondChallengesList(user_id, ChallengeConstants.INITIAL,offset).enqueue(new Callback<List<ChallengeSummary>>() {
            @Override
            public void onResponse(Call<List<ChallengeSummary>> call, Response<List<ChallengeSummary>> response) {
                if(response.isSuccessful()){
