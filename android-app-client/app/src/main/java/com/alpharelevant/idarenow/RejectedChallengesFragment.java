...
public class RejectedChallengesFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v =  inflater.inflate(R.layout.fragment_rejected_challenges, container, false);
        session = new Session(getContext());
        user_id = session.getUserID();

        rvItems = v.findViewById(R.id.RejectedChallengesRecyclerView);
        linearLayoutManager = new LinearLayoutManager(getContext());
        rvItems.setLayoutManager(linearLayoutManager);
        RetrofitClient.apiServices().getChallengesList(user_id, ChallengeConstants.REJECTED,0).enqueue(new Callback<List<ChallengeSummary>>() {
            @Override
    ...
    public void loadNextDataFromApi(int offset) {
        ...
        RetrofitClient.apiServices().getChallengesList(user_id, ChallengeConstants.REJECTED,offset).enqueue(new Callback<List<ChallengeSummary>>() {
            @Override
            public void onResponse(Call<List<ChallengeSummary>> call, Response<List<ChallengeSummary>> response) {
                if(response.isSuccessful()){
