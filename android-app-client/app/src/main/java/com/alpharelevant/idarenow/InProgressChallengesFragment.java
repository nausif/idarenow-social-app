...
public class InProgressChallengesFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v =  inflater.inflate(R.layout.fragment_in_progress_challenges, container, false);

        session = new Session(getContext());
        user_id = session.getUserID();

        rvItems = v.findViewById(R.id.inProgressChallengesRecyclerView);
        linearLayoutManager = new LinearLayoutManager(getContext());
        rvItems.setLayoutManager(linearLayoutManager);
        RetrofitClient.apiServices().getInProgressList(user_id, ChallengeConstants.ACCEPTED,0).enqueue(new Callback<List<ChallengeSummary>>() {
            @Override
    ...
    public void loadNextDataFromApi(int offset) {
        ...
        RetrofitClient.apiServices().getInProgressList(user_id, ChallengeConstants.ACCEPTED,offset).enqueue(new Callback<List<ChallengeSummary>>() {
            @Override
            public void onResponse(Call<List<ChallengeSummary>> call, Response<List<ChallengeSummary>> response) {
                if(response.isSuccessful()){
